...
func (d *Client) RunContainer(app *database.App, imageName string) (string, error) {
	logrus.Infof("Running container for app %s from image %s on port %d", app.Name, imageName, app.Port.Int64)
	...
	logrus.Infof("Container running successfully: %s (ID: %s, Port: %d)", imageName, resp.ID, app.Port.Int64)
	return resp.ID, nil
}
...
func (d *Client) buildHostConfig(app *database.App) *container.HostConfig {
	portBinding := nat.PortBinding{
		...
		HostPort: fmt.Sprintf("%d", app.Port.Int64),
	}
	// Use the internal port from the app model for the binding
	internalPort := nat.Port(fmt.Sprintf("%d/tcp", app.Port.Int64))
	portMap := nat.PortMap{
...
func (d *Client) buildContainerConfig(app *database.App, imageName string) *container.Config {
	...
	return &container.Config{
		...
		Env: []string{
			fmt.Sprintf("PORT=%d", app.Port.Int64),
		},
